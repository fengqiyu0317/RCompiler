...
echo "Starting tests with executable parser..."
echo "====================================="
...
for test_dir in $test_dirs; do
    ...
    # Run the parser executable and capture output
    $PARSER_EXEC < "$A_RS_PATH" > "test_results/${test_name}.out" 2>&1
    exit_code=$?
...
# Show some examples of failed tests if any
if [ $failed_tests -gt 0 ]; then
    echo ""
    echo "Sample failures:"
    for result_file in test_results/*.out; do
        test_name=$(basename "$result_file" .out)
        if [ ! -f "test_results/${test_name}.result" ]; then
            echo "=== $test_name ==="
            head -10 "$result_file"
            echo ""
        fi
