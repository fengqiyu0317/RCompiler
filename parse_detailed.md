...
2. **输入验证**
   - 检查是否有可用于解析的token
   - 如果没有token，抛出断言错误："No more tokens to parse in expression without block"

3. **检查ExprWithBlock起始标志**
   - 获取当前token
   - 如果当前token是"if"、"while"、"loop"或"{"：
     - 直接调用`parseExprWithBlockNode()`解析表达式
     - 递减递归深度
     - 返回解析结果
...
5. **解析后缀表达式**
   - 循环解析后缀表达式直到遇到不是后缀运算符的token：
     - **字段访问或方法调用**：如果当前token是"."：
       - 消费点
       - 使用`parsePathExprSegNode()`解析路径段
       - 如果下一个token是"("，那么它是方法调用：
         - 创建一个MethodCallExprNode
         - 将接收者设置为当前表达式
         - 将方法名设置为解析的路径段
...
public ExprWithoutBlockNode parseExprWithoutBlockNode(int precedence) {
    ...
    try {
        ...
        // 3. 检查ExprWithBlock起始标志
        if (token.name.equals("if") || token.name.equals("while") || token.name.equals("loop") || token.name.equals("{")) {
            // 当检测到exprwithblock起始标志时，直接调用parseExprWithBlockNode()
            recursionDepth--;
            return (ExprWithoutBlockNode) parseExprWithBlockNode();
        }
...
1. **优先检查ExprWithBlock起始标志**：
   - 在解析任何其他表达式类型之前，首先检查当前token是否是ExprWithBlock的起始标志
   - 如果是，直接调用`parseExprWithBlockNode()`并返回结果

2. **修改parseExprWithoutBlockNode()返回类型**：
   - 将`parseExprWithoutBlockNode()`的返回类型从`ExprWithoutBlockNode`改为`ExprNode`
   - 这样可以直接返回`ExprWithBlockNode`或`ExprWithoutBlockNode`，因为它们都是`ExprNode`的子类

3. **递归深度管理**：
   - 在调用`parseExprWithBlockNode()`前递减递归深度
   - 确保递归计数正确

### 注意事项

1. **返回类型统一**：`parseExprWithoutBlockNode()`现在返回`ExprNode`，可以包含`ExprWithBlockNode`或`ExprWithoutBlockNode`
2. **错误处理**：确保在检测到不支持的token时抛出明确的错误消息
3. **性能考虑**：直接调用`parseExprWithBlockNode()`避免了不必要的解析尝试，提高了性能
