...
public class StatementTypeChecker extends VisitorBase {
    ...
    public void visit(FunctionNode node) throws RuntimeException {
        try {
            // Enter function context in expression checker
            ...
            if (node.body != null) {
                ...
                Type bodyType = node.body.getType();

                System.err.println("Debug: Function '" + (node.name != null ? node.name.name : "anonymous") + "' expected return type: " + expectedReturnType + ", body type: " + bodyType);
                ...
                if (bodyType == null || expectedReturnType == null || !TypeUtils.isTypeCompatible(bodyType, expectedReturnType)) {
                    // Try to find a common type
                    Type commonType = TypeUtils.findCommonType(bodyType, expectedReturnType);
                    if (commonType == null || !TypeUtils.isTypeCompatible(commonType, expectedReturnType)) {
            ...
            // Exit function context in expression checker
        ...
        try {
            ...
            if (node.value != null) {
                ...
                if (node.type != null) {
                    ...
                    if (valueType == null || declaredType == null || !TypeUtils.isTypeCompatible(valueType, declaredType)) {
                        // Try to find a common type
                        Type commonType = TypeUtils.findCommonType(valueType, declaredType);
                        if (commonType == null || !TypeUtils.isTypeCompatible(commonType, declaredType)) {
                            RuntimeException error = new RuntimeException(
    ...
    public void visit(FieldNode node) {
        ...
        // The field type is processed when struct type is created
        // Just need to visit type if it exists
        if (node.type != null) {
    ...
    public void visit(ParameterNode node) {
        ...
        // Just need to visit pattern and type if they exist
        if (node.name != null) {
