...
public class Parser {
    ...
    public ExprNode parseExprNode(int precedence) {
        ...
        if (token.name.equals("if") || token.name.equals("while") || token.name.equals("loop") || token.name.equals("{")) {
            ...
            // Check the next token to determine if this is actually part of a larger expression
            if (i < tokens.size()) {
    ...
    private boolean isOperatorToken(String tokenName) {
        // Check if token is an operator that indicates this is part of a larger expression
        // Based on grammar.md, these tokens indicate without block expression starts
    ...
    public ExprWithoutBlockNode parseExprWithoutBlockNode(int precedence) {
        ...
        try {
            ...
            // Check for ExprWithBlock starting tokens
            if (token.name.equals("if") || token.name.equals("while") ||
                token.name.equals("loop") || token.name.equals("{")) {
                // Parse as ExprWithBlockNode and wrap in ExprWithoutBlockNode
                ExprWithBlockNode withBlockNode = parseExprWithBlockNode();
            ...
            else if (token.tokentype == token_t.TokenType_t.INTEGER_LITERAL ||
        ...
        return node;
            ...
            // Ensure recursion depth is decremented even in case of exception
            recursionDepth--;
            throw e;
        }
    }
