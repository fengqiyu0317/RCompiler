...
public class StringLiteralProcessor {
    ...
    public static boolean isStringStart(char c, String input, int position) {
        if (c == DOUBLE_QUOTE) return true;
        if (c == LETTER_R && position + 1 < input.length() && input.charAt(position + 1) == HASH) return true;
        if (c == LETTER_C && position + 1 < input.length() && input.charAt(position + 1) == DOUBLE_QUOTE) return true;
        if (c == LETTER_C && position + 2 < input.length() &&
            input.charAt(position + 1) == LETTER_R && input.charAt(position + 2) == HASH) return true;
        return false;
    }
    ...
    public static token_t createStringToken(String input, int position) {
        ...
        } else if (c == LETTER_C && input.charAt(position + 1) == DOUBLE_QUOTE) {
            return createCStringToken();
        } else if (c == LETTER_C && input.charAt(position + 1) == LETTER_R) {
